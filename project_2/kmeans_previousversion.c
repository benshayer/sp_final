...
static PyObject* fit_capi(PyObject *self, PyObject *args)
{
    ...
    printf("CONVERT DATA_POINTS");
    convert_data_to_c(centroids_py,centroids,k,d);
    printf("convert centroids");
...
static void convert_data_to_c(PyObject* data_points_py, double** data_points, int n, int d){
    ...
    for (i=0;i<n;i++){
        ...
        for(j=0;j<d;j++){
            ...
            printf("The item of point in place %d is %f\n",j,current_item);
            data_points[i][j] = current_item;
            printf("Finish set data point iteration %d \n",j);
        }
        free(point);
...
static void convert_point_to_c(PyObject* point_py, double* point, int d){
    ...
    for (i=0;i<d;i++){
        ...
        // printf("get item number %d",i);
        temp = PyFloat_AsDouble(item);
        // printf("%f",temp);
        // printf("%f",point[i]);

        printf("Values before change here: \n");
        printf("%f\n", point[i]);

        point[i] = temp;

        printf("Values after change here: \n");
        printf("%f\n", point[i]);
