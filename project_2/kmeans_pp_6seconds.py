...
def calculate_points_distance(row_i, row_j):
    #vector = data_points.loc[row_i]
    #centroid = data_points.loc[row_j]
...
def calculate_initial_centroids(k, data_points):
    ...
    while z != k - 1:
        ...
        for curr_row in range(n):
            current_vector = data_points.loc[curr_row]
            curr_distance = calculate_points_distance(current_vector, new_centroid)
            if curr_distance < min_distances[curr_row]:
                min_distances[curr_row] = curr_distance
